...
class ZirpuSpider(scrapy.Spider):
    name = 'zirpu'
    allowed_domains = ['www.lightinthebox.com']
    start_urls = ['https://www.lightinthebox.com/c/sports-outdoors_229']

# def start_requests(self):
#     yield SplashRequest(
#         url='https://www.lightinthebox.com/see-all.html',
#         callback=self.parse
#     )
    ...
    def parse(self, response):
        ...
        for url in response.css('dd.prod-name a::attr(href)').extract():
            yield scrapy.Request(url = url,callback=self.parse_details) 

        # for url in response.css('dl.item-block a::attr(href)').extract():
